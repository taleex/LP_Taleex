...
const ExperienceCard = ({ experience, index, isExpanded, onToggle }: ExperienceCardProps) => {
  return (
    <Collapsible open={isExpanded} onOpenChange={onToggle}>
      <div 
        className="group theme-bg-card backdrop-blur-sm theme-border border-2 rounded-3xl overflow-hidden transition-all duration-500 animate-fade-in-up"
      ...
      >
          ...
          <div className="p-8 cursor-pointer w-full text-left">
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
              <div className="flex-1">
                <div className="mb-3">
                  <h3 className="text-2xl md:text-3xl font-bold theme-text mb-2 transition-colors">
                    {experience.title}
                  </h3>
              ...
              <div className="flex items-center justify-center lg:justify-end">
                <div className="w-10 h-10 rounded-full theme-bg-muted flex items-center justify-center transition-colors duration-300">
                  {isExpanded ? (
                    <ChevronUp size={20} className="theme-text-muted" />
                  ) : (
        ...
        <CollapsibleContent className="overflow-hidden will-change-[height,opacity] data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
          <div 
            className="px-8 pb-8 pt-4 theme-border border-t-2 cursor-pointer transition-colors"
            onClick={onToggle}
          >
            ...
            <ul className="space-y-4">
              {experience.achievements.map((achievement, achIndex) => (
                <li key={achIndex} className="flex items-start gap-4 theme-text-muted transition-colors">
                  <div className="w-2.5 h-2.5 rounded-full bg-[#FF6542] mt-2 flex-shrink-0 transition-transform"></div>
