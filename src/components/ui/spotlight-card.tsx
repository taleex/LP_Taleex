...
interface GlowCardProps {
  ...
  className?: string;
  style?: CSSProperties;
  glowColor?: 'blue' | 'purple' | 'green' | 'red' | 'orange' | 'primary' | 'secondary';
  size?: 'sm' | 'md' | 'lg';
  width?: string | number;
...
const glowColorMap = {
  ...
  primary: { base: 12, spread: 50 }, // Using theme primary color (hsl(12 88% 63%))
  secondary: { base: 348, spread: 50 } // Using theme secondary color (hsl(348 49% 37%))
};
...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handlePointerLeave = () => {
      ...
      card.style.removeProperty('--xp');
      card.style.removeProperty('--y');
      card.style.removeProperty('--yp');
  ...
  const getInlineStyles = (): CSSProperties => {
    const baseStyles: CSSProperties & Record<string, any> = {
      ...
      '--saturation': glowColor === 'primary' || glowColor === 'secondary' ? '88' : '100',
      '--lightness': glowColor === 'primary' ? '63' : glowColor === 'secondary' ? '37' : '70',
      '--bg-spot-opacity': '0',
